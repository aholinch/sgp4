...
class SGP4 {
    ...
    static let WGS72OLD:Int = 1
    static let WGS72:Int = 2
    static let WGS84:Int = 3
    ...
    *    vallado, crawford, hujsak, kelso  2006
    ...
    static func dpper ( _ e3:Double, _ ee2:Double, _ peo:Double, _ pgho:Double, 
          _ pho:Double, _ pinco:Double, _ plo:Double, _ se2:Double, _ se3:Double, 
          _ sgh2:Double, _ sgh3:Double, _ sgh4:Double, _ sh2:Double, _ sh3:Double,
          _ si2:Double, _ si3:Double, _ sl2:Double, _ sl3:Double, _ sl4:Double, 
          _ t:Double, _ xgh2:Double, _ xgh3:Double, _ xgh4:Double, _ xh2:Double, 
          _ xh3:Double, _ xi2:Double, _ xi3:Double, _ xl2:Double, _ xl3:Double, 
          _ xl4:Double, _ zmol:Double, _ zmos:Double, _ initc:Character, _ rec: inout ElsetRec, _ opsmode:Character)
    {
        /* --------------------- local variables ------------------------ */
        var alfdp, betdp, cosip, cosop, dalf, dbet, dls:Double
        var f2, f3, pe, pgh, ph, pinc, pl:Double
        var sel, ses, sghl, sghs, shll, shs, sil:Double
        var sinip, sinop, sinzf, sis, sll, sls, xls:Double
        var xnoh, zf, zm:Double
    ...
    *    vallado, crawford, hujsak, kelso  2006
    ...
    static func dscom (_ epoch:Double, _ ep:Double, _ argpp:Double, _ tc:Double, _ inclp:Double,
        _ nodep:Double, _ np:Double, _ rec: inout ElsetRec)
    {
        /* -------------------------- constants ------------------------- */
        let zes:Double = 0.01675
        let zel:Double = 0.05490
        ...
        var a8, a9, a10, betasq, cc, ctem, stem:Double
        var x1, x2, x3, x4, x5, x6, x7:Double
        var x8, xnodce, xnoi, zcosg, zcosgl, zcosh, zcoshl:Double
        var zcosi, zcosil, zsing, zsingl, zsinh, zsinhl, zsini:Double
        var zsinil, zx, zy:Double
    ...
    *    vallado, crawford, hujsak, kelso  2006
    ...
    static func dsinit ( _ tc:Double, _ xpidot:Double, _ rec: inout ElsetRec)
    {
        /* --------------------- local variables ------------------------ */

        var ainv2, aonv, cosisq, eoc, f220, f221, f311:Double
        ...
        var ses, sgs, sghl, sghs, shs, shll, sis:Double
        var sini2, sls, temp, temp1, theta, xno2, q22:Double
        var q31, q33, root22, root44, root54, rptim, root32:Double
        var root52, x2o3, znl, emo, zns, emsqo:Double
    ...
    *    vallado, crawford, hujsak, kelso  2006
    ...
    static func dspace(_ tc:Double, _ rec: inout ElsetRec)
    {
        var iretn:Int
        var delt, ft, theta, x2li, x2omi, xl, xldot, xnddt, xndt, xomi, g22, g32:Double
        var g44, g52, g54, fasx2, fasx4, fasx6, rptim, step2, stepn, stepp:Double
    ...
    *    vallado, crawford, hujsak, kelso  2006
    ...
    static func initl(_ epoch:Double, _ rec: inout ElsetRec)
    {
        /* --------------------- local variables ------------------------ */
        ...
        var ds70:Double
        var ts70, tfrac, c1, thgr70, fk5r, c1p2p:Double
        ...
        c1p2p = c1 + TWOPI;
        var gsto1 = fmod(thgr70 + c1*ds70 + c1p2p*tfrac + ts70*ts70*fk5r, TWOPI);
        if (gsto1 < 0.0)
        {
            gsto1 = gsto1 + TWOPI;
        }
    ...
    *    vallado, crawford, hujsak, kelso  2006
    ...
    static func sgp4init ( _ opsmode:Character, _ satrec: inout ElsetRec) -> Bool
    {
        /* --------------------- local variables ------------------------ */
        
        var cc1sq, cc2, cc3, coef, coef1, cosio4:Double
        var eeta, etasq, perige, pinvsq, psisq, qzms24:Double
        var sfour,tc, temp, temp1, temp2, temp3, tsi, xpidot:Double
        var xhdot1,qzms2t, ss, x2o3:Double
        var delmotemp, qzms2ttemp, qzms24temp:Double
        ...
        satrec.am = 0.0; satrec.em = 0.0; satrec.im = 0.0; satrec.o_m = 0.0; satrec.mm = 0.0; satrec.nm = 0.0;
        ...
        if ((satrec.omeosq >= 0.0) || (satrec.no_unkozai >= 0.0))
        {
            ...
            if ((2 * PI / satrec.no_unkozai) >= 225.0)
            {
                ...
                dpper(satrec.e3, satrec.ee2, satrec.peo, satrec.pgho,
                    ...
                    satrec.xl3, satrec.xl4, satrec.zmol, satrec.zmos, satrec.initChar, &satrec,
    ...
    *    vallado, crawford, hujsak, kelso  2006
    ...
    static func sgp4 (_ satrec: inout ElsetRec, _ tsince:Double, _ r: inout [Double], _ v: inout [Double]) -> Bool
    {
        
        var axnl, aynl, betal, cnod:Double
        var cos2u, coseo1, cosi, cosip, cosisq, cossu, cosu:Double
        var delm, delomg, ecose, el2, eo1:Double
        var esine, argpdf, pl, mrt:Double
        var mvt, rdotl, rl, rvdot, rvdotl:Double
        var sin2u, sineo1, sini, sinip, sinsu, sinu:Double
        var snod, su, t2, t3, t4, tem5, temp:Double
        var temp1, temp2, tempa, tempe, templ, u, ux:Double
        var uy, uz, vx, vy, vz:Double
        var xinc, xincp, xl, xlm:Double
        var xmdf, xmx, xmy, nodedf, xnode, tc:Double
        var x2o3, vkmpersec, delmtemp:Double
        ...
        satrec.am = 1.0*satrec.am;
        satrec.em = 1.0*satrec.em;
        satrec.im = 1.0*satrec.inclm;
        satrec.o_m = 1.0*satrec.nodem;
        satrec.om = 1.0*satrec.argpm;
        satrec.mm = 1.0*satrec.mm;
        satrec.nm = 1.0*satrec.nm;
    ...
    *    vallado, crawford, hujsak, kelso  2006
    ...
    static func getgravconst(_ whichconst:Int32, _ rec: inout ElsetRec)
    {
        rec.whichconst = whichconst;
        switch Int(whichconst)
        {
    ...
    static func fmod(_ numer:Double, _ denom:Double) -> Double
    {
        let tquot = floor(numer/denom)
    ...
    static func jday(_ year:Int, _ mon:Int, _ day:Int, _ hr:Int, _ minute:Int, _ sec:Double) -> (jd:Double, frac:Double){
        var jd:Double = 0
        var jdFrac:Double = 0

        let dyear = Double(year)
        let dmon = Double(mon)
        let dday = Double(day)
        let dhr = Double(hr)
        let dminute = Double(minute)

        jd = 367.0 * dyear -
             floor((7 * (dyear + floor((dmon + 9) / 12.0))) * 0.25) +
                        floor(275 * dmon / 9.0) +
                        dday + 1721013.5  // use - 678987.0 to go to mjd directly
        jdFrac = (sec + dminute * 60.0 + dhr * 3600.0) / 86400.0
